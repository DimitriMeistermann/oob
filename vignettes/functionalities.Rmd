...
- `assoc2vector`: Associate each pair of level from two vectors pf multilevel 
	  factor of the same length. The typical use case is to associate clusters to
	  samples in the context of scRNA-Seq.
- `to_clipboard`: Copy data to the clipboard, for quickly pasting somewhere else!
- `diff_abundance`: Propoper differential abundance in the context of scRNA-Seq.
