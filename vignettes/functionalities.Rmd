...
ggBorderedFactors(
	ggplot(resPCR,aes(x=PC,y=Rsquared,fill=Annotation))+
		...
		scale_fill_manual(values = oobColors(nlevels(resPCR$Annotation)))+
		theme(
...
```
With `neighborFeatureGrid` we can identify wich gene is marking which area of a projection (here the UMAP).

```{r out.width = "100%", fig.dim = c(20,20)}
neighborFeatureGrid(coordUMAP,featureMat = bulkLogCounts)
```
...
Finally, `customUpsetPlot` is used to compare intersection between gene set with an additional value of enrichment.
