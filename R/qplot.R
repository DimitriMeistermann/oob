...
#' @param returnGraph Logical. Logical. Return the graph as a ggplot object
...
#' This function is a copy of ggplot2::qplot, as it is deprecated since ggplot2
...
#' @param facets faceting formula to use. Picks [facet_wrap()] or [facet_grid()]
...
#'   if x and y are specified, and "histogram" if only x is specified.
#' @param xlim,ylim X and y axis limits
#' @param log Which variables to log transform ("x", "y", or "xy")
#' @param main,xlab,ylab Character vector (or expression) giving plot title, x
...
oobqplot<-function (x, y, ..., data, facets = NULL, margins = FALSE,
            geom = "auto", xlim = c(NA, NA), ylim = c(NA, NA),
            log = "", main = NULL, xlab = NULL, ylab = NULL, asp = NA)
{
...
rename_aes<-function (x) {
    ...
    if (length(duplicated_names) > 0L) {
        cli::cli_warn(
            paste0("Duplicated aesthetics after name standardisation: ",
                            "{.field {unique0(duplicated_names)}}")
        )
    }
    x
}
...
stop_input_type<-function (x, what, ..., allow_na = FALSE, allow_null = FALSE,
                        show_value = TRUE, arg = caller_arg(x),
                        call = caller_env()) {
...
check_character<-function (x, ..., allow_null = FALSE, arg = caller_arg(x),
                        call = caller_env()){
    if (!missing(x)) {
        if (is_character(x)) {
            return(invisible(NULL))
...
obj_type_friendly<-function (x, value = TRUE){
    ...
    if (!n_dim) {
        if (!is_list(x) && length(x) == 1) {
            if (is_na(x)) {
                return(switch(typeof(x), logical = "`NA`",
...
.rlang_as_friendly_type<-function (type) {
    switch(type, list = "a list", `NULL` = "`NULL`",
...
vec_type_friendly<-function (x, length = FALSE)
{
    ...
    type <- switch(type, logical = "a logical %s", integer = "an integer %s",
                ...
                character = "a character %s", raw = "a raw %s",
