...
#'
#' 1. A data frame (*enrichStats*) containing the enrichment statistics for each intersection of sets.
...
#'
#' 2. A combination matrix (*upsetMatrix*), can be used as input to [ComplexHeatmap::UpSet]
#'
...
#' The main ones are a p-value for each overlap, and the effect size of the association as the OEdev: `(observed - expected) / sqrt(universeSize)`
#'
#'
#' @return
            ...
            %v% Heatmap(
                ...
                cell_fun = function(j, i, x, y, w, h, col) {
                    if (colSums(col2rgb(col)) < 382.5)
                        ...
                        col = "black"
                    grid.text(as.character(signif(regionEnrichRes[j, "OEdeviation"], 2)), x, y, gp = gpar(col = col, fontface =
                                                                                                                    2))
                },
