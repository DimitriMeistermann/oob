...
#' @param squareHt Logical or NULL. Apply clustering columns on rows. If NULL automatically turned TRUE if `ncol==nrow` and col/rownames are the same.
...
#'
#' bestDE <-
#'     rownames(DEgenesPrime_Naive)[whichTop(DEgenesPrime_Naive$pvalue,
#'                                           decreasing = FALSE,
#'                                           top = 50)]
#' heatmap.DM(
...
#' heatmap.DM(bulkLogCounts[bestDE[seq_len(5)],
#'     rownames(sampleAnnot)[sampleAnnot$culture_media %in% c("T2iLGO","KSR+FGF2")],
#'     ])
#'
...
#'     preSet = "cor",
#'     center = TRUE,
#'     colorScaleFun = circlize::colorRamp2(c(-0.2, 0, 0.2), c("blue", "white", "red"))
#' )
heatmap.DM <-
	function(matrix,
					 preSet = "expr",
		...
		if (is.null(squareHt)) {
			if (nrow(matrix) == ncol(matrix) &
				...
				squareHt <- TRUE
				print("colnames and rownames are identical, squareHt is set to TRUE")
			} else{
				squareHt <- FALSE
			}
		}
...
#'
#' @param countTable A matrix of numeric with samples as columns (in the RNA-Seq context, log counts)
...
htMarker <-
						...
						...)
	{
		...
		if(is.null(colnames(countTable)))
			colnames(countTable) <- make.names(paste0("X",seq_len(ncol(countTable))))
		names(group) <- colnames(countTable)
		if(!all(rownames(markerData) %in% rownames(countTable))) {
			stop("rownames of markerData must exist in the rownames of countTable")
		}

		drawPerGroup<-drawSamplePerGroup(names(group),group, minDrawSize = maxDrawSize,
...
#'         right_annotation  = genRowAnnot(sampleAnnot[, c("culture_media", "line")]))
genRowAnnot <-
	function (annot,
						colorScales = NULL,
						border = TRUE,
