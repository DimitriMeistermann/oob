...
#' - sdev: the standard deviations of the principal components (i.e., the square roots of the eigenvalues of the covariance/correlation matrix, though the calculation is actually done with the singular values of the data matrix).
#' - rotation: the matrix of variable loadings (i.e., a matrix whose columns contain the eigenvectors). The function princomp returns this in the element loadings.
#' - x: if retx is true the value of the rotated data (the centered (and scaled if requested) data multiplied by the rotation matrix) is returned. Hence, cov(x) is the diagonal matrix diag(sdev^2). For the formula method, napredict() is applied to handle the treatment of values omitted by the na.action.
#' - center, scale: the centering and scaling used, or FALSE.
#' - n.obs: Number of samples
#' - propExplVar: proportion of explained variance by principals components on total variance.
#' - transform: a list containing the scaling (sdeviations) and centering factors (means) for each principal component.
...
PCR <- function(pca, annotationDF, nComponent = 10) {
	...
	for (x in cn(pca$x)) {
		for (annot in annots) {
			rSquaredMat[x, annot] <-
				summary(lm(formula(paste0(
					x, "~", annot
				)), data = data.frame(annotationDF[, annot, drop =
	...
	if (ncol(annotationDF) < 2) {
		...
		retDt <-
			reshape2::melt(
				rSquaredMat[seq_len(nComponent), ],
				value.name = "Rsquared",
	...
	retDt$PC <-
		factor(retDt$PC, levels = cn(pca$x)[seq_len(nComponent)]) #so the levels of PCs are well ordered
	retDt
}
...
PCaov <- function(pca, colData, nComponent = 10) {
	...
	anovPerPCdt <-
		do.call("rbind", lapply(colnames(pca$x[, seq_len(nComponent)]), function(PC) {
			anova_model <-
	...
	anovPerPCdt$PC <-
		paste0("PC",
					 formatNumber2Character(substr(anovPerPCdt$PC, 3, nchar(anovPerPCdt$PC))))
	return(anovPerPCdt)
}
...
#' @param nDimPCA Integer. If not NULL compute a PCA first and take n first principal components.
#' @param transpose Logical. If `transpose`, samples are columns and features are rows.
