...
PCR <- function(pca, annotationDF, nComponent = 10) {
    ...
    if (ncol(annotationDF) < 2) {
        ...
        retDt <-
            reshape2::melt(rSquaredMat[seq_len(nComponent),],
                                         value.name = "Rsquared",
...
extractFeatureMarkerData <- function(markerData, feature = "score") {
    ...
    columns2Pick <-
        grep(paste0("^.*\\.", feature, "$"), colnames(markerData), value = TRUE)
...
#' irisUMAP<-UMAP(rowScale(iris[,seq_len(4)],center = TRUE,scaled = TRUE),transpose = FALSE,n_neighbors = nrow(iris),ret_nn = TRUE)
#' proj2d(irisUMAP$embedding,colorBy = iris$Species,nnMatrix = irisUMAP$nn$euclidean$idx[,c(1, 2, 3)],fixedCoord = TRUE)
UMAP <-
    function(data,
                     nDimPCA = NULL,
                     transpose = TRUE,
