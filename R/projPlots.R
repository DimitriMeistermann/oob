...
#' @param plotVars Logical. Plot variable loadings instead of coordinates of
#'   samples on PCs.
#' @param pointSize Single numeric. Size of points.
#' @param plotText Logical. Plot sample/variable name instead of point.
#' @param ratioPerPercentVar Logical. Ratio of the plot is corresponding to the
#'   percentage of explained variation from each PC.
#' @param main Single character. Main title of the graph.
#' @param ellipse Logical. Add stat_ellipse to the projection (eventually
#'   several ellipses if colorBy is a factor).
#' @param colorScale A vector of colors. Equal to the desired number of breaks
#'   for continuous values or number of levels for factors.
#' @param returnGraph Logical. Return the graph as a ggplot object instead of
#'   printing it.
#' @param outlierLabel Logical. Add a `ggrepel` label for point in low-density
#'   area of the plot.
#' @param outlierLabelThres Numeric. Density value threshold below that the
#'   labelled is displayed if `outlierLabel=TRUE`.
...
pca2d <-
    function(pca,
        ...
        graph <-
            ...
            xlab(paste0("PC", comp[1], ": ", round(propExplVar[comp[1]] * 100),
                        varianceMesage)) +
            ylab(paste0("PC", comp[2], ": ", round(propExplVar[comp[2]] * 100),
                        varianceMesage))
        if (outlierLabel) {
...
#' @param ellipse Logical. Add stat_ellipse to the projection (eventually
#'   several ellipses if colorBy is a factor).
#' @param emph Single character or `NULL`. Must be a level of `colorBy` if it is
#'   a factor. Emphasize a particular value on the plot.
#' @param colorScale A vector of colors. Equal to the desired number of breaks
#'   for continuous values or number of levels for factors.
#' @param returnGraph Logical. Return the graph as a ggplot object instead of
#'   printing it.
#' @param legendTitle Character. The legend title on the plot.
#' @param axis.names Vector of two characters. Axis names (x, y) showed on the
#'   plot.
...
#' @param plotFactorsCentroids Logical. If `ColorBy` is a factor Display the
#'   name of the factor at the centroid of its points.
#' @param pointStroke Numeric. Width of points border.
#' @param strokeColor Character. Color of points border.
#' @param funAutoColorScale A function that return n colors with n as a first
#'   argument. Responsible for color mapping of the levels.
#' @param fixedCoord Logical. The ratio of the plot is representative of the
#'   ratio between x and y values.
#' @param plotLabelRepel Logical. Plot the rownames of the coordinates as a
#'   ggrepel label of each point.
#' @param labelSize Numeric. Size of the labels if `plotLabelRepel=TRUE`.
#' @param sizeProp2Dens Logical. Size of point is inversely proportional to the
#'   2D density of its area.
#' @param densEps Numeric. Radius of the eps-neighborhood, i.e., bandwidth of
#'   the uniform kernel). Used if `sizeProp2Dens=TRUE`.
#' @param nnMatrix A matrix of integer, number of row equal to the number of
#'   points, as returned by `make.umap(..., ,ret_nn = TRUE)`. Plot segments
#'   between the neighbors.
#' @param nnSegmentParam List of arguments for customizing the nn segments.
#'   Passed to `geom_segment`.
#' @param useScatterMore Logical. Use `geom_scatter` for making the plot. Render
...
proj2d <-
    ...
    if (plotFactorsCentroids) {
        ...
        centroids <-vapply(samplePerFactor, function(x)
            colMeans(d[x, c("Axis1", "Axis2")]),numeric(2)) |> t()

        graph <-
            graph + geom_label(
                data = data.frame(
                        centroids, groupName = names(samplePerFactor)
                    ),
                mapping = aes(
                    x = .data$Axis1,
                    y = .data$Axis2,
...
#' proj1d(iris$Sepal.Length, variable.name = "Length of sepal" ,
#'     main = "Two features from iris")
#'
#'
#' proj1d(
...
proj1d <-
    ...
    if (plotFactorsCentroids) {
        ...
        graph <-
            graph + geom_label(
                data = data.frame(centroids,
                    groupName = names(samplePerFactor)),
                mapping = aes(
                    x = .data$Axis1,
                    y = .data$Axis2,
                    label = .data$groupName
...
#'
#' @param coord Matrix or dataframe containing at least three vectors of numeric
#'   corresponding to the x/y/z coordinates.
#' @param axis A vector of 3 integer. The column index of `coord` containing
#'   respectively the x and y coordinates.
#' @param colorBy A vector of factor or numeric same size as number of samples
#'   in PCA. Color each point on the projection by its corresponding value.
#' @param pointSize Single numeric. Size of points.
...
#'
#' proj3d(iris,colorBy = iris$Species,colorScale =
#'   c("blue","white","red"),pointSize = .05)
proj3d <-
    function(coord,
            axis = c(1, 2, 3),
            colorBy = NULL,
...
barplotPercentVar <-
    function(pca,
            ...
            returnGraph = FALSE) {
    g <- qplotBarplot(pca$propExplVar[seq_len(nPC)] * 100, returnGraph = TRUE) +
